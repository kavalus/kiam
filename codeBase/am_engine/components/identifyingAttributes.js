...
async function FindAttribute(attribute){
 ...
 async function FindResource(id,resname){
    let policyObj = hashTable.get('policyObj');
    let res = resname;
    let attr_id = id;
    console.log("Id23 : "+id)
// console.log(appid,"77777777777777777777")
  //console.log("val : " + appid._id + "--" + res + "--" + attr_id);
    let foundResource = await (Resource.findOne({'application_id':policyObj.application_id,'res_name':res,'attributes':{$elemMatch:{'attribute_id':attr_id.toString()}}},{'attributes.$.attribute_value':1,'_id':0},function(err, result) {
        if(err) console.log(err);
