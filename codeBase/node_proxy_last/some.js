        ...
        login.checklogin(req.body.username, req.body.password, url).then((data) => {
            ...
            if (data != true) {
                ...
                login.auth_factor(url).then((auth_type_res) => {
                    ...
                    request({ method: 'POST', url: 'http://localhost:4000/api/scan', form: { username: "omkar", qr_code: 'data' } }, ( res, err) => {
                        if (err) {
                            res.send(err);
                        }
                        else{
                        ...
                        // res.send("hello")
                        res.sendFile(path.join(__dirname + '/qrgenenerate.html'));
                    }
                    })
...
app.get('/post/generate/'+abc, (req, res) => {
